...
public class DatacenterController extends DatacenterBroker implements GeoLocatable,
	...
	public DatacenterController(String name, 
		...
		} else if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_THROTTLED)) { //i.e. if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_THROTTLED))
			this.loadBalancer = new ThrottledVmLoadBalancer(this);
		} 
		else if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_INTRA)){
			this.loadBalancer = new IntraBalancer(this);
		} 
		else if (loadBalancePolicy.equals(Constants.LOAD_BALANCE_INTRA_RL)){
			this.loadBalancer = new IntraBalancer(this);
		}
		else {
			 throw new IllegalArgumentException("Invalid load balancing policy: " + loadBalancePolicy);
		}
